> **ℹ️ GENERATED ARTIFACT — a snapshot of one run, not a design document.**
> Regenerated by the driver's `--inspect-n` flag. Numbers here reflect only the
> cells in that run; the results of record are `data/final_run_results.md`.
