...
| 14 | metric-analysis input set | **ten published metrics decomposed alone, ρ_F held out and projected, then added (two-stage, since 2026-09-16)** | all fourteen metrics at once, ρ_F and the FI variants included (`data/factor_audit.md`) | Horn retains 3 on the fourteen only because AUFI (≡ accuracy) and the FI premium add success indicators. On the eight published metrics alone it retains 2 (third eigenvalue 0.83 < 1). Adding the published best- and worst-formulation accuracies F_max, F_min (computed from the graded rates; the persisted `spread` keeps the pipeline's original scoring) retains success but still not dependence (1.09 vs threshold 1.26): spread and ρ_u correlate only +.42. Adding ρ_F retains 3 (1.61 vs 1.29, stable over n = 56–150 and ten seeds); held out, ρ_F projects +.89 on the published dependence component (spread +.74, ρ_u +.84 active) and ≤ +.10 elsewhere. Source: `data/metric_selection.md`. |
