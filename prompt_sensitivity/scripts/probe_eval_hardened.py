...
from ..feedback.heads import DEFAULT_LAYER_FRACS, FeedbackModel, build_features, head_labels
...
def permutation_null(head_fn, X, y, qids, *, n_perm, seed=42, score="auroc") -> np.ndarray:
    ...
    for i in range(n_perm):
        yp = permute_labels_by_question(y.astype(float), qids,
...
def eval_model_in_distribution(config, model_key, *, n_perm_mass=200,
                               n_perm_logistic=25, seed=42) -> pd.DataFrame:
    ...
    for target, (binary, perm_ok) in _TARGETS.items():
        ...
        for lf in layer_fracs:
            ...
            if binary:
                ...
                null_m = (permutation_null(
                    lambda X_, y_, q_, seed: roc_null_scores_massmean(X_, y_, q_, seed),
                    Xk, yk, qk, n_perm=n_perm_mass, seed=seed)
                    if perm_ok else flip_control(oof_m, yk, qk, seed=seed))
                real_m = roc_auc_score(yk, oof_m)
                real_l = roc_auc_score(yk, oof_l)
                p_m = float((np.sum(null_m >= real_m) + 1) / (len(null_m) + 1))
                rows.append(dict(
                    model=model_key, target=target, layer_frac=lf, head="massmean",
                    auroc=real_m, null_mean=float(null_m.mean()),
                    null_sd=float(null_m.std()), null_p=p_m,
                    null_kind="permutation" if perm_ok else "flip", n=len(yk),
                ))
                rows.append(dict(
                    model=model_key, target=target, layer_frac=lf, head="logistic",
                    auroc=real_l, null_mean=np.nan, null_sd=np.nan, null_p=np.nan,
                    null_kind="(null at selected layer only)", n=len(yk),
                ))
                ...
                null_r = permutation_null(
                    ...
                    Xk, yk, qk, n_perm=max(10, n_perm_logistic), seed=seed,
                    score="spearman")
                rows.append(dict(
                    model=model_key, target=target, layer_frac=lf, head="ridge",
                    auroc=real, null_mean=float(null_r.mean()),
                    null_sd=float(null_r.std()),
                    null_p=float((np.sum(null_r >= real) + 1) / (len(null_r) + 1)),
                    null_kind="permutation(spearman)", n=len(yk),
                ))
        ...
        rows.append(dict(
            model=model_key, target=target, layer_frac=np.nan, head="nested_cv",
            auroc=nested["score"], null_mean=np.nan, null_sd=np.nan, null_p=np.nan,
            null_kind=f"layer selected in inner CV (mode {sel_layer})", n=len(y0),
        ))
        ...
        if binary:
            ...
            if perm_ok:
                null_l = permutation_null(
                    ...
                    Xsel, ysel, qsel, n_perm=n_perm_logistic, seed=seed)
            else:
                null_l = flip_control(per_layer_oof[sel_layer], ysel, qsel, seed=seed)
            real_l = roc_auc_score(ysel, per_layer_oof[sel_layer])
            rows.append(dict(
                model=model_key, target=target, layer_frac=sel_layer,
                head="logistic+null", auroc=real_l, null_mean=float(null_l.mean()),
                null_sd=float(null_l.std()),
                null_p=float((np.sum(null_l >= real_l) + 1) / (len(null_l) + 1)),
                null_kind="permutation" if perm_ok else "flip", n=len(ysel),
            ))
        ...
        if binary:
            for kind in ("length", "tfidf_word", "tfidf_char", "first_word"):
                ...
                a = (_auc_best_orientation(yb, s) if kind == "length"
                     else roc_auc_score(yb, s))
                rows.append(dict(
                    model=model_key, target=target, layer_frac=np.nan,
                    head=f"baseline_{kind}", auroc=float(a), null_mean=np.nan,
                    null_sd=np.nan, null_p=np.nan, null_kind="", n=len(yb),
                ))
...
def nested_layer_selection(Xs: dict, y, qids, *, binary, seed=42) -> dict:
    ...
    for tr, te in group_folds(qids, 5, seed=seed):
        ...
        for lf in layer_fracs:
            Xin = Xs[lf][tr]
            s = (massmean_oof(Xin, y[tr], q_tr, n_splits=3, seed=seed) if binary
                 else ridge_oof(Xin, y[tr], q_tr, n_splits=3, seed=seed))
            score = (roc_auc_score(y[tr], s) if binary
                     else stats.spearmanr(y[tr], s).statistic)
            if score > best:
    ...
    score = (float(roc_auc_score(y, oof)) if binary
             else float(stats.spearmanr(y, oof).statistic))
...
def eval_ood(config, model_key) -> dict:
    ...
    lab = (hold.drop_duplicates(["question_id"])
           .set_index("question_id")["ambiguous"].astype(int))
    meta["y"] = meta.question_id.map(lab)
    y = meta.y.to_numpy(dtype=int)
    scores = bundle.heads["vagueness"].predict(X)

    out = {"model": model_key, "n": int(len(y)), "n_ambiguous": int(y.sum())}
    ...
    out.update(dict(threshold=_SHIPPED_THRESHOLD, tp=tp, fp=fp, fn=fn, tn=tn,
    ...
    for kind, vec in [("tfidf_word", TfidfVectorizer(ngram_range=(1, 2), min_df=2,
                                                     sublinear_tf=True)),
...
def render(indist: pd.DataFrame, ood: list[dict]) -> str:
    ...
    L.append("Controls at EVERY layer (null distributions, not single draws); layer selection inside")
    L.append("nested CV; text baselines under matched protocols; operating-point analysis on the OOD")
    ...
    L.append("")
    L.append("| model | AUROC head | PR-AUC (prev.) | length | TF-IDF w/c (frozen) | first-word (frozen) |")
    L.append("|---|---|---|---|---|---|")
    for o in ood:
        L.append(f"| {o['model']} | {o['auroc_head']:.3f} | {o['prauc_head']:.3f} "
    ...
    L.append("|---|---|---|---|---|")
    for o in ood:
        L.append(f"| {o['model']} | {o['flagged_frac']:.1%} | {o['precision']:.3f} | "
                 f"{o['recall']:.3f} | {o['tp']}/{o['fp']}/{o['fn']}/{o['tn']} |")
    L.append("")
    L.append("**Claim discipline:** the head's OOD advantage is over *frozen* baselines "
...
def main() -> int:
    ...
    for m in args.models:
        df = eval_model_in_distribution(config, m, n_perm_mass=args.n_perm_mass,
                                        n_perm_logistic=args.n_perm_logistic)
        df.to_parquet(config.repo_root() / f"data/probe_eval_hardened_{m}.parquet",
                      index=False)
        frames.append(df)
